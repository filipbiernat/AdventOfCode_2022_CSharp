    ...
    public class Day5B : IDay
    {
        public void Run()
        {
            ...
            foreach (var step in rearrangementProcedure)
            {
                ...
                Stack<char> multipleCratesToRearrange = new();
                foreach (int crate in Enumerable.Range(0, howMany))
                {
                    multipleCratesToRearrange.Push(stacksOfCrates[moveFrom].Pop());
                }
                foreach (int crate in Enumerable.Range(0, howMany))
                {
                    stacksOfCrates[moveTo].Push(multipleCratesToRearrange.Pop());
                }
            }
